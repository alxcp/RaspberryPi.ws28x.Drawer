...
class FireEffect(MatrixPixelEffect):
    ...
    def play_frame(self):
        ...
        for x in range(self.width):
            position = super(FireEffect, self).translateToMatrixColumns(x, self.height - 1)
    ...
    def process_spark(self, spark):
        ...
        #spark.color = ColorRGB(255 / ((spark.altitude + 1) * 5), 0, 0)
        
    def draw_spark(self, spark):
        for y in range(0, int(spark.altitude)):
            position = super(FireEffect, self).translateToMatrixColumns(spark.position, self.height - 1 - y)
