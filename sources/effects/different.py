...
class RainbowHSVEffect(PixelEffect):
    def __init__(self, drawer, frame_delay = 0):
        ...
        self.translate = 0
        self.p = 1.0 / (self.drawer.nLED * 1) * 1
        print(self.p)
...
class RainbowHSV2Effect(PixelEffect):
    def __init__(self, drawer, frame_delay = 0):
        PixelEffect.__init__(self, drawer, frame_delay)
        self.nLED = drawer.nLED
        self.translate = 0
        self.translate_incr = 1
        self.p = 1.0 / (self.nLED * 5)
    ...
    def play_frame(self):
        ...
        if self.translate == self.drawer.nLED * 30:
            self.translate_incr = -1
...
class RandomBlinksEffect(PixelEffect):
    ...
    def play_frame(self):
        if (len(self.blinks) < self.maxLeds):
            ...
            blink = Blink(random.randint(0, self.nLED - 1), blink_color, 10)
...
class ColorBounceEffect(PixelEffect):                         #-m5-BOUNCE COLOR (SINGLE LED)
    ...
    def play_frame(self):
        ...
        for i in range (0, drawer.nLED - 1):
            if i == idex:
                self.drawer.set_color(i, ColorRGB.CHSV(drawer.nLED, i, self.drawer.intensity_max, 1))
            else:
...
class FullFadeEffect(PixelEffect):    
    ...
    def play_frame(self):
        ...
        if self.current_step == self.steps - 30:
            ...
            self.bounce_direction = 1
            targetColor = ColorRGB.random(2, self.drawer.intensity_min, self.drawer.intensity_max)
                    
            stepR = targetColor.r / float(self.steps)
            stepG = targetColor.g / float(self.steps)
            stepB = targetColor.b / float(self.steps)

        self.current_step += self.bounce_direction
        currentColor = ColorRGB(
            stepR * self.current_step, 
            stepG * self.current_step, 
            stepB * self.current_step)

        for z in range(0, self.drawer.nLED - 1):
            self.drawer.set_color(z, currentColor)
...
class WavesEffect(PixelEffect):
    ...
    def play_frame(self):
        if self.current_cycle >= self.cycles:
            ...
            if self.transition_cycle == self.drawer.nLED:
                self.transition_cycle = -1
...
class RainbowWavesEffect(PixelEffect):
    def __init__(self, drawer, steps, cycles, frame_delay = 0):
        ...
        self.current_cycle = 0
        self.transition_cycle = -1
        self.p = 1.0 / (drawer.nLED * 5)
        self.intensity_max = drawer.intensity_max

    def play_frame(self):
        if self.current_cycle >= self.cycles:
            self.transition_cycle += 1
            if self.current_cycle - self.cycles == drawer.nLED:
                self.current_cycle = 0
...
class DropsEffect(PixelEffect):
    ...
    def play_frame(self):
        ...
        for drop in self.drops:
            drop[0] = drop[0] + 1
            if drop[0] == self.drawer.nLED + self.tail_len:
                delete_first = True
            else:
                faid = self.tail_len
                for z in range(drop[0] - self.tail_len - 1 - self.drops_distance, drop[0]):
                    if z > 0 and z < self.drawer.nLED - 1:
                        if faid <= 0:
                            pixel = ColorRGB()
                        else:
